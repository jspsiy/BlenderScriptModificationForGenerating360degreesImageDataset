...
def handle_found_object(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if (
            ...
            (len(png_files) != 6*num_renders)
            or (len(npy_files) != 6*num_renders)
...
def render_objects(
    ...
    # Filter for GitHub-related sources
    github_df = annotations[annotations['source'].str.contains("github", case=False, na=False)]

    # Sample 300 rows per group within the GitHub-related sources
    sampled_df = github_df.groupby('source').apply(lambda x: x.sample(300)).reset_index(drop=True)
    objects = sampled_df
